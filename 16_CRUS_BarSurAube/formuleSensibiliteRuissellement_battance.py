...
fichierRasterPentes = "note_pente.tif"
fichierRasterPermea = "permea.tif"
fichierRasterOccSol_5a = "occupation_sol.tif"
...
t2 = threading.Thread(target=warp_raster, args=(fichierRasterOccSol_5a, "occsolrepr.tif", [xmin, ymin, xmax, ymax], resX, resY, ))
...
t2.join()
log(fichier_log, f"...Fin du découpage occsol - temps total = {datetime.datetime.now() - t0}")
t3.join()
...
rasterOccSol_5a = gdal.Open("occsolrepr.tif").ReadAsArray()
...
dataOccSol = rasterOccSol_5a.reshape(dim1*dim2)
...
#  ###########################################################################
log(fichier_log, f"...Début du Calcul - temps total = {datetime.datetime.now() - t0}")
suff = ".tif"
if dimtot > dimmax:
    nb = dimtot // dimmax
    ...
    dataResult_sp = dataOccSol.copy()
    data_Ssbatt_sp = dataOccSol.copy()
    dataDiff = dataOccSol.copy()
    data_Ssbatt = dataResult.copy()
    debut = datetime.datetime.now()
    threadsperblock = 1024
    blockspergrid = math.ceil(dataResult.shape[0] / threadsperblock)
    my_kernel[blockspergrid, threadsperblock](dataPentes,dataPermea,dataOccSol,dataBattance,dataResult,data_Ssbatt,dataDiff,tabCorCod,tabCoefBat, dataResult_sp, data_Ssbatt_sp)
    log(fichier_log, f"...Traitement GPU terminé en : {datetime.datetime.now() - debut}")
...
log(fichier_log, f"...Calcul terminé - temps total de traitement : {datetime.datetime.now() - t0}")
